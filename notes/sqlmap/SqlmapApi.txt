...
public class SqlmapApi
{
	...
	public async Task<bool> SetOptionAsync(string taskid, string option, object value)
	{
		string json = "{ ";
		...
		JToken response = JObject.Parse(await sqlmapRequest.PostAsync($"/option/{taskid}/set", json));
	...
	{
		JToken response = JObject.Parse(await sqlmapRequest.PostAsync($"/scan/{taskid}/start", "{ }"));
		var success = response.SelectToken("success");
		if (success != null)
		{
			return (bool)success;
